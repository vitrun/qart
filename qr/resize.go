...
// average convert the sums to averages and returns the result.
func average64(sum []uint64, w, h int, n uint64) *image.RGBA {
	ret := image.NewRGBA(image.Rect(0, 0, w, h))
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			index := 4 * (y*w + x)
...
// ResizeRGBA returns a scaled copy of the RGBA image slice r of m.
// The returned image has width w and height h.
// hacked from ResizeRGBA
// FIXME. convert RGBA64 TO RGBA
func ResizeRGBA64(m *image.RGBA64, r image.Rectangle, w, h int) *image.RGBA {
	ww, hh := uint64(w), uint64(h)
	dx, dy := uint64(r.Dx()), uint64(r.Dy())
	// See comment in Resize.
	n, sum := dx*dy, make([]uint64, 4*w*h)
	...
	return average64(sum, w, h, (uint64)(n))
}
